...
export default function YourPosts() {
    ...
    const postData = async () => {
        try {
            const res = await axios.get(`http://localhost:8000/yourpost?id=${pid}`);
            setTweet(res.data);
        }
        catch (err) {
            console.log(err);
    ...
    const handleDelete = async (id) => {
        const del = await axios.post("http://localhost:8000/deletepost", { id });
        if (del.data.success) {
                            ...
                            tweet.map(tweetItem => (
                                <div className="bg-white mt-3" key={tweetItem._id}>
                                    <div className="bg-white border border-gray-200 shadow-md p-4 rounded-lg flex items-start">
                                        <img
                                            src={`http://localhost:8000/${tweetItem.photo}`}
                                            alt="User Avatar"
                                    ...
                                    <img className="border rounded-t-lg shadow-lg" src={`http://localhost:8000/${tweetItem.location}`} alt="post" />
