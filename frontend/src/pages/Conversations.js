...
export default function Conversations() {
    ...
    const sendChat = async () => {
        try {
            const res = await axios.post("http://localhost:8000/sendmessage", { senderId, receiverId, message });
        } catch (error) {
            console.log(error);
        }
    }
    ...
    const getMessages = async () => {
        try {
            const mess = await axios.post("http://localhost:8000/getmessages", { senderId, receiverId });
            setChats(mess.data);
            console.log(chats);
        } catch (error) {
            console.log(error);
