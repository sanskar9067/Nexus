...
export default function Followers() {
    ...
    const fetchFollowers = async () => {
        try {
            const res = await axios.post("http://localhost:8000/getfollowers", { id });
            if (res.data.success) {
        ...
        <div>
            ...
            <div className="w-full bg-indigo-100 h-screen flex flex-row flex-wrap justify-center">
                ...
                <div className="w-full md:w-3/4 lg:w-4/5 p-5 md:px-12 lg:24 h-full overflow-x-scroll antialiased">
                    <h1 className="text-center mb-4 text-4xl font-bold">Followers</h1>
                    {followers && followers.followers && followers.followers.length > 0 ? (
                            ...
                            <div className="flex justify-center" key={follower.id}>
                                ...
                                <div class="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow my-4">
                                    ...
                                    <div class="flex flex-col items-center pb-10">
                                        <img class="w-24 h-24 mb-3 rounded-full shadow-lg" src={`http://localhost:8000/${follower.profilePhoto}`} alt="Bonnie image" />
