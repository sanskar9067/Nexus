...
export default function Comments() {
    ...
    const fetchDetails = async () => {
        const res = await axios.get(`http://localhost:8000/getpostdetails/${id}`);
        setDetails(res.data);
    }
    ...
    const handleComment = async (e) => {
        e.preventDefault();
        try {
            const comm = await axios.post("http://localhost:8000/addcomment", { id, name, comment });
                ...
                {details && (
                    <div className="max-w-xl bg-white rounded-lg shadow-lg p-6">
                        ...
                        <div className="mt-4">
                            ...
                            <img className="mt-4 rounded-lg shadow-lg w-full h-72 object-cover" src={`http://localhost:8000/${details.location}`} alt="Post" />
                        </div>
                        <div className="mt-4">
