...
export default function Userprofile() {
    ...
    const fetchData = async () => {
        try {
            const res = await axios.get(`http://localhost:8000/userdetails/${id}`);
            if (res.data.success) {
                setDetails(res.data);
            }
            console.log(details);
    ...
    const likePost = async (actualid, postid) => {
        try {
            console.log(postid);
            console.log(lid);
            const res = await axios.post("http://localhost:8000/like", { actualid, postid, lid });
            if (res.data.success) {
                ...
                <div className="w-full bg-indigo-100 h-screen flex flex-row flex-wrap justify-center">
                    ...
                    <div className="w-0 md:w-1/4 lg:w-1/5 h-0 md:h-screen overflow-y-hidden bg-white shadow-lg">
                        <div className="p-5 bg-white sticky top-0">
                            <img className="border border-indigo-100 shadow-lg round" src={`http://localhost:8000/${details.user.location}`} alt="Profile" />
                                    ...
                                    details.posts.map(posts => (
                                        <div className="bg-white rounded-lg shadow-md mb-4" key={posts._id}>
                                            <div className="p-4 flex items-center">
                                                <img
                                                    src={`http://localhost:8000/${posts.photo}`}
                                                    alt="User Avatar"
                                            ...
                                            {posts.location && (
                                                <img
                                                    ...
                                                    src={`http://localhost:8000/${posts.location}`}
                                                    alt="post"
                                                />
                                            )}
