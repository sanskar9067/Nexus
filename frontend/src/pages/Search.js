...
export default function Search() {
    ...
    const handleSearch = async (e) => {
        ...
        try {
            console.log(user);
            const res = await axios.post("http://localhost:8000/searchuser", { user });
            if (res.data.success) {
    ...
    const handleFollow = async (userId, userName, userProfilePhoto, userEmail) => {
        try {
            const follow = await axios.post("http://localhost:8000/follow", { authId, userId, authName, userName, authProfilePhoto, userProfilePhoto, authEmail, userEmail });
            ...
            <div className="w-full bg-indigo-100 h-screen flex flex-row flex-wrap justify-center">
                ...
                <div className="w-0 md:w-1/4 lg:w-1/5 h-0 md:h-screen overflow-y-hidden bg-white shadow-lg">
                    <div className="p-5 bg-white sticky top-0">
                        <img className="border border-indigo-100 shadow-lg round" src={`http://localhost:8000/${auth.user.location}`} alt="Profile" />
                            ...
                            <div className='flex justify-center mt-5'>
                                <div className="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow">
                                    <div className="flex flex-col items-center pb-10 pt-4">
                                        <img className="w-24 h-24 mb-3 rounded-full shadow-lg" src={`http://localhost:8000/${usedet.data.user[0].location}`} alt="Bonnie image" />
